	...
	// SEMI-PARAMETRIC
		...
		if(`tvc'==. & "`chk_tGaps'"==""){
            ...
            *********************************************************************************************************************
            ...
            local currentSort: sortedby 
            reset_stset, id(`stID_ch') sort(`sorter') newsort("`currentSort'")
...
qui{
    syntax, [ID(string) SORT(string) newsort(string)]
    
    // To address the irregular "could not restore sort order because 
    // variables were dropped" FYI
    sort `sort'
    ...
    * multiple_options (since most extensive): 13 possibilities
    ...
    // Re-sort to the order prior to calling reset_stset
